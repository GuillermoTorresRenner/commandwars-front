...
interface InspirationCardFormDialogProps {
  open: boolean
...
export function InspirationCardFormDialog({
  open,
  onOpenChange,
  factionId,
  initial,
  onSaved,
}: InspirationCardFormDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{initial ? "Editar inspiración" : "Nueva inspiración"}</DialogTitle>
          <DialogDescription>Define los atributos de la inspiración: acción, requisitos y efectos data-driven.</DialogDescription>
        </DialogHeader>
        <InspirationCardForm
          key={initial?.id ?? "new"}
          factionId={factionId}
...
function InspirationCardForm({
  ...
  const [actionType, setActionType] = useState(initial?.actionType ?? "full")
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "No se pudo guardar la inspiración")
    } finally {
      setSaving(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto">
      ...
      <DialogFooter>
        ...
        <Button type="submit" disabled={saving}>
          {saving ? "Guardando…" : initial ? "Guardar cambios" : "Crear inspiración"}
        </Button>
      </DialogFooter>
    </form>
  )
